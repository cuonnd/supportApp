...
export const EditorTool: React.FC = () => {
  ...
  const handleTransform = async () => {
    ...
    try {
      ...
      const mimeType = sourceImage.substring(
  ...
  const handleDownload = () => {
    if (resultImage) {
      ...
      link.download = `WowArt Ai-edit-${Date.now()}.png`;
  ...
  return (
    <div className="max-w-5xl mx-auto w-full animate-fade-in-up">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold mb-2 bg-gradient-to-r from-accent-400 to-pink-400 bg-clip-text text-transparent">
          Smart Editor & Object Removal
        </h2>
        <p className="text-gray-400">
          Upload a photo and tell the AI what to change, remove, or transform.
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          {resultImage && (
            <div className="animate-fade-in">
              ...
              <ImageResult imageUrl={resultImage} onDownload={handleDownload} />
            </div>
          )}
        </div>
      </div>
